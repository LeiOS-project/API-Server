...
- Main boot is `src/index.ts`: load config -> init DB -> init permission helper -> start task queue -> start Aptly sidecar -> start Hono API.
...
- On empty DB, startup creates a default admin and writes a reset URL to `${LRA_CONFIG_BASE_DIR}/initial_admin_password_reset_token.txt`.
...
- Keep API response envelope consistent with `APIResponse` (`success`, `code`, `message`, `data`).
